    ...
    public class EditStudentViewModel : BaseViewModel
    {
        ...
        async Task EditStudent()
        {
            try
            {
                ...
                {
                    await Application.Current.MainPage.DisplayAlert("Error", "You must enter all the field", "Cancel");
                    return;
                }
                ...
                var studentDTO = new StudentDTO { ID = this.Student.ID, LastName = this.Student.LastName, FirstMidName = this.Student.FirstMidName, EnrollmentDate = this.Student.EnrollmentDate, FullName = this.Student.FullName };
