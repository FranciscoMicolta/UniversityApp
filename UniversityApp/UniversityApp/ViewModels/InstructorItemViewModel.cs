...
namespace UniversityApp.ViewModels
{
    public class InstructorItemViewModel : InstructorDTO
    {
        ...
        {
            var answer = await Application.Current.MainPage.DisplayAlert("Confirm", "Confirm", "Yes", "No");
            if (!answer)
                return;
            ...
            var instructorDelete = instructorViewModel.AllInstructor.FirstOrDefault(x => x.ID == this.ID);
            instructorViewModel.AllInstructor.Remove(instructorDelete);
            instructorViewModel.GetInstructorByName();
        }
    }
}
