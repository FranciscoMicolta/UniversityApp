...
namespace UniversityApp.ViewModels
{
    public class EnrollmentsViewModel : BaseViewModel
    {
        ...
        public EnrollmentsViewModel()
        {
            ...
            this.RefreshCommand = new Command(async () => await GetEnrollment());
            this.RefreshCommand.Execute(null);
        }

        public Command RefreshCommand { get; set; }

        async Task GetEnrollment()
        {
            try
            {
                ...
                var listEnrollment = await enrollmentService.GetAll(Endpoints.GET_ENROLLMENTS);
                this.Enrollment = new ObservableCollection<EnrollmentDTO>(listEnrollment);
                this.IsRefreshing = false;
            }
            catch (Exception ex)
            {
